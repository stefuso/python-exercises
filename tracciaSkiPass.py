...
def leggiPiste(filePiste):
    ...
    with open (filePiste, "r") as file:
        file.readline()
        for line in file:
            line=line.strip("\r\n").split(";")
            metri=int(line[2])
            lunghezza=arrotondaDistanze(metri)
            tupla=line[1],lunghezza
            line[0]=int(line[0])
            diz[line[0]]=tupla
...
def leggiPassaggi(nomeFilePassaggi):
    li=[]
    with open (nomeFilePassaggi, "r") as file:
        file.readline()
        for line in file:
            line=line.strip("\r\n").split(";")
            id=int(line[0])
            ...
            mm=int(line[2][1])
            idpista=int(line[3])
            puntibonus=puntiBonusPiste(gg,hh,mm)
            tupla=id,idpista,puntibonus
            li.append(tupla)
...
def calcolaClientiPiste(passaggi, datiPiste):
    ...
    for el in passaggi:
        idCli = el[0]
        if idCli not in datiCli:
            datiCli[idCli] = [0,0]
        punti = el[2]
        id_pista = el[1]
        metri = datiPiste[id_pista][1] #recupero la lunghezza della pista
        datiCli[idCli][0] += metri # aggiorno i metri del cliente
        datiCli[idCli][1] += punti # aggiorno i punti del cliente
    return datiCli
